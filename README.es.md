...
- **Complemento para Home Assistant**: este repositorio funciona además como repositorio de complementos
...
- **Backend Scamper (Linux)**: cada compilación para Linux enlaza estáticamente las bindings
  [`rscamper`](https://crates.io/crates/rscamper), por lo que el ping y el
  traceroute pueden enrutarse a través de un demonio scamper en ejecución
  configurando `"backend": "scamper"` en la medición. Los hosts sin scamper
  instalado no se ven afectados a menos que se seleccione realmente este
  backend. El socket por defecto es `/var/run/scamper/scamperd.sock` y puede
  anularse mediante `STARLA_SCAMPER_SOCKET`. No disponible en compilaciones
  para macOS o Windows.
...
# Paquete de la versión publicada
...
Añade `https://github.com/ananthb/starla` como repositorio de complementos
(**Ajustes → Complementos → Tienda de complementos → ⋮ → Repositorios**) y
después instala el complemento **Starla**. Consulta la
[documentación del complemento](starla/DOCS.md).
...
Starla habla los idiomas que aporte la gente: la aplicación de la bandeja
del sistema, este README y el sitio de documentación se traducen en
